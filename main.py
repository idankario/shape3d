...
class DrowingApp:
    ...
    def drawAgain(self):
        ...
        lines,circels,bezier
        draw(lines,circels,bezier)
    ...
    def draw(self,lines,circels,bezier):
        #Process work simultany
        thread_list = []
        for  line in lines:
            thread=threading.Thread(target=self.myLine, args=(line[0],line[1],line[2]))
        ...
        try:
            if tf:
                with open(tf) as f:
                    ...
                    self.fileOpend = f          
                self.draw( self.data['lines'],self.data['circles'],self.data['bezier'])
            elif tf=='':
